...
class WarpDataset(BaseDataset):
    ...
    def process_disparity(self, disparity, max_disparity_range=(40, 196)):
        ...
        disparity /= (disparity.max() + 1e-5)  # now 0-1
        

        scaling_factor = (max_disparity_range[0] + random.random() / 10 *
    ...
    def process_disparity_2(self, disparity, image,  max_disparity_range=(40, 196), bound=0.1):
        """ Depth predictions have arbitrary scale - need to convert to a pixel disparity"""
        ...
        if not self.disable_sharpening:
            disparity = sparse_bilateral_filtering( disparity.copy(), image.copy(), filter_size=[5, 5], num_iter=1, )
    ...
    def augment_synthetic_image(self, image):
        ...
        # color augmentation
        stereo_aug = transforms.ColorJitter(
            self.stereo_brightness, self.stereo_contrast, self.stereo_saturation,
            self.stereo_hue)
    ...
    def __getitem__(self, idx):
        ...
        # inputs['disparity'] = \
        #     self.process_disparity_2(inputs['loaded_disparity'],
        #                              inputs['left_image'],
        #                              max_disparity_range=(50, self.max_disparity))
