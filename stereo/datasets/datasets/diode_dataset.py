...
class DiodeDataset(WarpDataset):
    ...
    def __init__(self,
        ...
        elif self.monodepth_model == 'dan':
            self.disparity_path = 'DepAny_depth'
        elif self.monodepth_model == 'mix':
            self.disparity_path = random_str(['midas_depths', 'DepAny_depth'])
        else:
    ...
    def load_images(self, idx, do_flip=False):
        ...
        split, realm, scene, scan, loc, angle = self.filenames[idx]
        ...
        split, realm, scene, scan, loc, angle = random.choice(self.filenames)
    ...
    def load_disparity(self, idx, do_flip=False):
        split, realm, scene, scan, loc, angle = self.filenames[idx]
        disparity = np.load(os.path.join(self.data_path, str(self.disparity_path), split, realm,
                                         'scene_{}'.format(scene),
