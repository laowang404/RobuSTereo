...
from .datasets.ade20k_dataset import ADE20KDataset as _ADE20KDataset
...
from .datasets.mapillary_dataset import MapillaryDataset as _MapillaryDataset
import yaml


class ADE20KDataset(DatasetTemplate):
    def __init__(self, data_info, data_cfg, mode):
        ...
        self.dataset = _ADE20KDataset(self.root,
    ...
    def __getitem__(self, idx):
        ...
        # print(sample['left'].max(), sample['right'].max())
        # print(sample["left"].shape, sample["right"].shape, sample["disp"].shape)
        # print("before transform", sample['left'].max(), sample['left'].min())
        sample = self.transform(sample)
        # print("after transform", sample['left'].max(), sample['left'].min())
        sample['index'] = idx
