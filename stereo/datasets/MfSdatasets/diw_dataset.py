...
class DIWDataset(WarpDataset):
    ...
    def __init__(self,
        ...
        data_path = PATH_CONFIG['diw']
        filenames = readlines(os.path.join('/data/wangyuran/DepthGen/stereo-from-mono/splits/diw', filenames))
    ...
    def load_images(self, idx, do_flip=False):
        ...
        image = self.loader(get_left_view(os.path.join(self.data_path, file)))

        background = self.loader(get_right_view(os.path.join(self.data_path, file)))
        ...
        if self.return_path:
            return image, background, os.path.join(self.data_path, file)
    ...
    def load_disparity(self, idx, do_flip=False):
        ...
        disparity = np.load(get_disp_view(os.path.join(self.data_path, file)))
