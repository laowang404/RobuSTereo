...
class VirtualKitti2Dataset(DatasetTemplate):
    def __init__(self, data_info, data_cfg, mode):
        ...
        self.data_num = self.data_info.get("DATA_NUM", len(self.data_list)) if mode == 'training' else len(self.data_list)

    def __len__(self):
        return self.data_num
    
    def __getitem__(self, idx):
        # return self.my_get_item(idx + np.random.randint(0, len(self.data_list) // self.data_num) * self.data_num)
        return self.my_get_item(len(self.data_list) - (idx + np.random.randint(0, len(self.data_list) // self.data_num) * self.data_num) - 1)
