...
class RandomEraseVertical(object):
    def __init__(self, config):
        self.eraser_aug_prob = config.PROB
        self.max_erase_time = config.MAX_TIME
        self.bounds = config.BOUNDS
    ...
    def __call__(self, sample):
        ...
        if np.random.rand() < self.eraser_aug_prob:
            ...
            for _ in range(np.random.randint(1, self.max_erase_time + 1)):
                ...
                dx = np.random.randint(self.bounds[0], self.bounds[1])
                # dy = np.random.randint(self.bounds[0], self.bounds[1])
                img2[:, x0:x0 + dx, :] = mean_color
                img1[:, x0:x0 + dx, :] = mean_color
...
class RightTopPad(object):
    ...
    def __call__(self, sample):
        ...
        for k in sample.keys():
            ...
            elif k in ['disp', 'disp_right', 'occ_mask', 'occ_mask_right', 'mono']:
...
class RandomFlip(object):
    ...
    def __call__(self, sample):
        ...
        if np.random.rand() < self.prob and self.flip_type == 'horizontal':  # 水平翻转
            ...
            mono = np.ascontiguousarray(mono[:, ::-1] * -1.0) if mono is not None else None
        ...
        if np.random.rand() < self.prob and self.flip_type == 'horizontal_swap':  # 水平翻转并交换
            ...
            img2 = tmp
            mono = np.ascontiguousarray(mono[:, ::-1]) if mono is not None else None
        ...
        sample['right'] = img2
        sample['disp'] = disp
        sample['mono'] = mono if mono is not None else None
        return sample
...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        for k in sample.keys():
            if k in ['left', 'right']:
                sample[k] = cv2.resize(sample[k], (tw, th), interpolation=cv2.INTER_LINEAR)
            if k in ['mono', 'disp']:
                sample[k] = cv2.resize(sample[k], (tw, th), interpolation=cv2.INTER_NEAREST)
            if k in ['mono', 'disp']:
                sample[k] = sample[k] * disp_scale
            
        return sample
    
class DivisibleResize(object):
    def __init__(self, config):
        self.by = config.BY
    ...
    def __call__(self, sample):
        h ,w = sample['left'].shape[:2]
        if h % self.by != 0:
            resize_h = self.by - h % self.by + h
        else:
            resize_h = h
        if w % self.by != 0:
            resize_w = self.by - w % self.by + w
        else:
            resize_w = w

        for k in sample.keys():
            if k in ['left', 'right', 'mono']:
                sample[k] = cv2.resize(sample[k], (resize_w, resize_h), interpolation=cv2.INTER_LINEAR)
            if k in ['disp', 'disp_right', 'occ_mask', 'occ_mask_right', 'mono']:
                #use padding 
                pad_h = resize_h - h
                pad_w = resize_w - w
                pad_top = pad_h // 2
