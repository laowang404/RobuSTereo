...
def backup_source_code(backup_dir):
    ...
    ignore_hidden = shutil.ignore_patterns(
        ".idea", ".git*", "*pycache*",
        "cfgs", "data", "output","pretrain","docs","deploy","png","*.ipynb")
...
def write_wandb(wdb_writer, tb_info, step):
    if wdb_writer is None:
        return
    for k, v in tb_info.items():
        info_type = k.split('/')[0]
        info_from = k.split('/')[1]
        info_name = k.split('/')[1:]
        board_name = '/'.join(info_name)
        v = v.detach() if torch.is_tensor(v) else v
        if info_type == 'image':
            # import pdb; pdb.set_trace()
            wdb_writer.log({board_name: wandb.Image(v), info_from + '_step': step}, commit=False)   
        else:
            wdb_writer.log({board_name: v, info_from + '_step': step}, commit=False)
    wdb_writer.log({},commit=True)
